...
def get_infer_results(outs, catid, bias=0):
    """
    Get test_results at the stage of inference.
    The output format is dictionary containing bbox or mask test_results.

    For example, bbox test_results is a list and each element contains
    image_id, category_id, bbox and score.
    """
    if outs is None or len(outs) == 0:
        raise ValueError(
            'The number of valid detection test_results if zero. Please use reasonable model and check input data.'
        )
...
def cocoapi_eval(jsonfile,
    ...
    if classwise:
        ...
        logger.info("per-category PR curve has output to {} folder.".format(
            style + '_pr_curve'))
    # flush coco evaluation test_results
    sys.stdout.flush()
