...
class WandbLogger():
    ...
    def download_dataset_artifact(self, path, alias):
        if path.startswith(WANDB_ARTIFACT_PREFIX):
            ...
            labels_zip = Path(datadir) / "data/test_labels.zip"
            shutil.unpack_archive(labels_zip, Path(datadir) / 'data/test_labels', 'zip')
    ...
    def log_dataset_artifact(self, dataset, class_to_id, name='dataset'):
        ...
        labels_path = 'test_labels'.join(image_path.rsplit('images', 1))
        ...
        artifact.add_file(str(zip_path), name='data/test_labels.zip')
    ...
    def finish_run(self):
        if self.wandb_run:
            if self.result_artifact:
                ...
                self.result_artifact.add(self.result_table, 'test_results')
