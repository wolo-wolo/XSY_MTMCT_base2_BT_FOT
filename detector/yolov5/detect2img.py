...
def detect(save_img=False):
    ...
    webcam = source.isnumeric() or source.endswith('.txt') or source.lower().startswith(
        ('rtsp://', 'rtmp://', 'http://'))  # False

    # Directories
    save_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
    # save_dir: /home/ubuntu/wgj/XSY_MTMCT_base1/datasets/detect_merge/c041-c46
    (save_dir / 'test_labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            txt_path = str(save_dir / 'test_labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
    ...
    if save_txt or save_img:
        s = f"\n{len(list(save_dir.glob('test_labels/*.txt')))} test_labels saved to {save_dir / 'test_labels'}" if save_txt else ''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-conf', action='store_true', help='save confidences in --save-txt test_labels')
    parser.add_argument('--classes', nargs='+', type=int, help='filter by class: --class 0, or --class 0 2 3')
    parser.add_argument('--agnostic-nms', action='store_true', help='class-agnostic NMS')
    parser.add_argument('--augment', action='store_true', help='augmented inference')
    parser.add_argument('--update', action='store_true', help='update all models')
    parser.add_argument('--project', default='runs/detect', help='save results to project/name')
    parser.add_argument('--name', default='exp_v7x', help='save results to project/name')
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment')
    parser.add_argument('--cfg_file', default='aic_mcmt.yml', help='Config file for mcmt')
    opt = parser.parse_args()
    cfg.merge_from_file(f'../../config/{opt.cfg_file}')  # aic_all.yml(prior)
    cfg.freeze()
    opt.source = f'{cfg.DET_SOURCE_DIR}/{opt.name}/img1/'
    opt.project = cfg.DATA_DIR # from aic_all.yml
    check_requirements()
