...
import warnings
warnings.filterwarnings("ignore")  # add by wgj

BATCH_SIZE = 448  # 64
NUM_PROCESS = 4  # aval cuda_gpu
...
def extract_image_feat(_cfg):
    ...
    image_list = load_all_data(_cfg.DET_IMG_DIR)  # './datasets/detect_merge/'
    ...
    # init reid model in multiprocess
    process_pool = Pool(processes=num_process, initializer=init_worker, initargs=(gpu_ids, _cfg,))
    start_time = time.time()

    # process_input_by_worker_process return reid feat_dict extracted by reid model from det(detections) image
