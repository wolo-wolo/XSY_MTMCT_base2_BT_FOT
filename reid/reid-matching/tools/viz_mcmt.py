...
def draw_bboxes(img, bbox, identities=None, offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        cv2.putText(
            img, label, (x1, y1 + t_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 4, [255, 255, 255], 3)
    return img


def viz_mcmt(args, cam, cam_track):
    print("start cam:{}".format(cam))
    # cam_dir = os.path.join(args.data_path, 'test', 'S06', 'c00{}'.format(cam))
    cam_dir = os.path.join(args.data_path, 'test_videos', 'S06', 'c00{}'.format(cam))
    cap = cv2.VideoCapture(os.path.join(cam_dir, "vdo.mp4"))
    out_dit = os.path.join(args.output_path, 'mcmt', 'S06')
    img_dir = os.path.join(args.output_path, 'mcmt', 'S06', 'imgs')
    if not os.path.isdir(img_dir):
        os.makedirs(img_dir)
    out = cv2.VideoWriter(os.path.join(out_dit, "c00{}.mp4".format(cam)),
    ...
    while (state):
        if fr_id in cam_track:
            ...
            cv2.putText(im, "%d" % (fr_id), (0, 50),
                        cv2.FONT_HERSHEY_COMPLEX, 4.0, (100, 200, 200), 3)
            # plt.imshow(im)
            # plt.show()
...
if __name__ == "__main__":
    ...
    args.output_path = cfg.MTMC_VIS_OUTPUT
