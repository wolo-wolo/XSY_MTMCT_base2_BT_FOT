...
# {(1, 2): 0, (2, 3): 1, (3, 4): 2, (4, 5): 3, (5, 6): 4, (6, 5): 5, (5, 4): 6, (4, 3): 7, (3, 2): 8, (2, 1): 9}
...
def calc_ious(atlwhs, btlwhs):
    ...
    """
    atlwhs_np = np.array(atlwhs, dtype=np.float64)
    btlwhs_np = np.array(btlwhs, dtype=np.float64)

    atlbrs = np.zeros_like(atlwhs_np, dtype=np.float64)
    btlbrs = np.zeros_like(btlwhs_np, dtype=np.float64)
    ...
    ious = np.zeros((atlbrs.shape[0], btlbrs.shape[0]), dtype=np.float64)
    if ious.size == 0:
        return ious
...
### calc time cost(diff of start fid betwenn cid of the smae tid )，and return c2c tuples of every tracklet
def calc_c2c_cost(tracklets: dict) -> dict:
    """
    return:
     tid_cid_costs:{tid:
                    {[cid, cid+1]: time diff(start fid in cid+1 - end fid in cid),
                    [cid+1, cid]: time diff(start fid in cid - end fid in cid+1),}
                    tidx...
                    }
     tid_c2c: {
                tid1: {[cid, cid+1],[cid+1, cid]...}
                tid2: {[cid, cid+1],[cid+1, cid]...}
              }
    """
    tid_cid_costs = {}
    for tid, cid_dict in tracklets.items():
        for cid in cid_dict.keys():
            ...
            if next_fid_list[0] < current_fid_list[0]:
                time_diff = abs(current_fid_list[0] - next_fid_list[-1]) / 25.0  # 25 is fps
                key_name = (cid + 1, cid)
            else:
                time_diff = abs(next_fid_list[0] - current_fid_list[-1]) / 25.0
                key_name = (cid, cid + 1)
...
def calc_c2c_traj_len(tracklets: dict) -> dict:
    ...
    return:
    tid_cid_traj_len:
    {'tid-X':{'cid-X':{'len': len(fid) or num of bbox, 'rects': bbox_list}}}
    """
    tid_cid_traj_len = {}
    for tid, cid_dict in tracklets.items():
        for cid in cid_dict.keys():
...
def prepare_cluster_inputs(costs: dict, speeds: dict):
    ...
    for tid in tc_cost_speed.keys():
        ...
        for i in range(num_tracklet):
            tvec = np.zeros((tvec_len), dtype=np.float64)  # vtec is the best!!!
            c2c = tc_cost_speed[tid]['c2c'][i]
            tvec[C2C_ONEHOT_MAP[c2c]] = 1.0
            tvec[tvec_len-3] = tc_cost_speed[tid]['time'][i]
            tvec[tvec_len-2] = tc_cost_speed[tid]['speed'][i][0]  # speed
            tvec[tvec_len-1] = tc_cost_speed[tid]['speed'][i][1]  # acceleration
    ...
    for tid, speed_seq in speed_dict.items():
        ...
        for cid in cid_list:
            ...
            if speed_seq[cid] == (0, 0) and cid != max(cid_list):
                if traj_len_dict[tid][cid]['len'] > 25:  # 25 is fps
...
def find_direct_outliers(cost_dict: dict) -> dict:
    ...
    for tid, time_seq in cost_dict.items():
        ...
        for i in range(len(c2c_list)):
            ...
            if (to_start and c2c[0] < c2c[1]) or (to_end and c2c[0] > c2c[1]):
                if i == 1:
        ...
        if is_conflit:
            c2c_main_direct = 0  # (to)start or (to)end or (Unknown)0
            if to_start_cnt > to_end_cnt:
                c2c_main_direct = 1
            elif to_end_cnt > to_start_cnt:
                c2c_main_direct = 6

            error_tid_c2c.setdefault(tid,
                                     {"c2c": tmp_ec2c_list, "cost": tmp_ec2c_costs_list, "direct": c2c_main_direct})
...
def find_overshort_outliers(traj_len_dict: dict, only_find_one_cam=False) -> dict:
    ...
    for tid, traj_len_seq in traj_len_dict.items():
        ...
        if only_find_one_cam:
            ...
            for cid in cid_list:
                if traj_len_seq[cid]['len'] < 25:  # 25 is fps
                    if tid in error_tid_traj:
...
def find_cluster_outliers(data, idx, n_clusters, is_scale=True):
    ...
    # 判断是否需要对数据做标准化处理
    if is_scale:
        std_data = scale(data[:, tvec_len-3:], axis=0)  # 标准化
        data = np.concatenate((data[:, :tvec_len-3], std_data), axis=1)
...
def find_cluster_outliers_2(data, idx, n_clusters, is_scale=True, method="kmeans"):
    if method == "kmeans":
        ...
        outlier_res = []
        if is_scale:
            std_data = scale(data[:, tvec_len-3:], axis=0)
            data = np.concatenate((data[:, :tvec_len-3], std_data), axis=1)
...
def fix_overshort_outlier(error_tid_traj_len: dict, tid_cid_dict: dict):
    ...
    # {684: {'rect': [259, 251, 205, 138], 'output': '6 92 684 259 251 205 138 -1 -1\n'},...}
    for e_tid, error_cids in error_tid_traj_len.items():
        # error_tid_traj_len: {8: {1: 5, 5: 3}, ...]  {Tid: {cid: len}}
        for e_cid in error_cids:
            ...
            if error_cids[e_cid] < 3:
                if e_cid in temp_tid_cid[e_tid].keys():
    ...
    for e_tid, error in error_tid_c2c.items():
        ...
        if e_direct != 0:
            for e_c2c_idx in range(len(e_c2c_list)):
                ...
                if e_c2c[0] < e_c2c[1] and e_direct == 1:
                    if len(e_c2c_list) == 1:
                        del temp_tid_cid[e_tid][e_c2c[1]]
                    else:
                        del temp_tid_cid[e_tid][e_c2c[0]]
                elif e_c2c[0] > e_c2c[1] and e_direct == 6:
                    if len(e_c2c_list) == 1:
...
if __name__ == "__main__":
    ...
    cluster_path = os.path.join('./', 'test_cluster.pkl')
    data_dir = cfg.DATA_DIR
    mot_dir = cfg.DATA_DIR
        ...
        for fid in img_rects:
            ...
            for tid_rect in tid_rects:
                ...
                if (cid, tid) in map_tid:
                    ...
                    output = str(cid) + ' ' + str(new_tid) + ' ' + str(fid) + ' ' + ' '.join(
                        map(str, new_rect)) + ' -1 -1' '\n'  # original mtmct result
                    if new_tid in tid_cid_dict:
                        ...
                        # tid_cid_dict{'new_tid1': {cid1: {fid1: {'rect': new_rect, 'output': output},
                        #                                fid2: {'rect': new_rect, 'output': output}
                        #                          ...}}
                        tid_cid_dict.setdefault(new_tid, {cid: {fid: {'rect': new_rect, 'output': output}}})

    """
    ### 过滤规则
    * 速度及加速度异常:非最后cam的tracklet若出现速度和加速度均为0的情况,则认为是异常情况,且后面的所有轨迹都被删除,因为出现速度和加速度均为0一般是stop car,若到视频结束，依旧未动，车辆不可能在下一个相邻cam出现；若出现速度为0，加速度非0情况，也认为是异常情况；
    ...
    * 单一镜头轨迹
    ...
    print(f"traj len outliers(single_cam): {error_tid_traj_len}")
    # fix the outlier
