"""modified by wgj: add use_camera to fit the case of wtih/without use_camera"""
...
def get_sim_matrix(_cfg, cid_tid_dict, cid_tids):
    ...
    use_st_filter = cfg.USE_ST_FILTER
    if use_st_filter:
...
def get_labels(_cfg, cid_tid_dict, cid_tids, score_thr):
    ...
    for i, sub_c_to_c in enumerate(sub_cid_tids):
        ...
        cluster_labels = AgglomerativeClustering(n_clusters=None, distance_threshold=1 - dis_thrs[i],
    ...
    #                                          linkage='complete').fit_predict(1 - sim_matrix)
    # test_labels = get_match(cluster_labels)
    return labels


def get_labels_without_camera(_cfg, cid_tid_dict, cid_tids,):
    """added by wgj"""
    # 1st cluster
...
if __name__ == '__main__':
    ...
    cfg.freeze()
    scene_name = ['S06']
    # scene_cluster = [[1, 2, 3, 4, 5, 6]]
    scene_cluster = [[1, 2, 3]]
    fea_dir = './exp_ori/viz/test/S06/trajectory/'
    cid_tid_dict = dict()
    ...
    # cid_tids is the sored list [(cid1,tid1)....(cidC,tidT)]
    # print(cid_tids)
    use_camera = cfg.USE_CAMERA  # added by wgj
    if use_camera:  # added by wgj
        clu = get_labels(cfg, cid_tid_dict, cid_tids, score_thr=cfg.SCORE_THR)  # ori
    else:  # added by wgj
        clu = get_labels_without_camera(cfg, cid_tid_dict, cid_tids)  # added by wgj
