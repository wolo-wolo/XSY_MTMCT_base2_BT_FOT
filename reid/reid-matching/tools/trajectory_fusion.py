"""modified by wgj: add use_zone to fit the case of with/without zones"""
...
def out_new_mot(mot_list, mot_path):
    out_dict = dict()
    for tracklet in mot_list:  # tracklet is the first key --TID--
...
if __name__ == '__main__':
    ...
    save_dir = './exp_ori/viz/test/S06/trajectory/'
    ...
    for cam_path in cam_paths:
        ...
        #
        # 'tid-i': {
        #    'fid-i': {bbox': (xyxy), 'frame': 'imgFID', 'id': TID-i, 'imgname', 'feat': array([2048-d]), 'zone': None}}
        #    'fid-j': {bbox': (xyxy), 'frame': 'imgFID', 'id': TID-i, 'imgname', 'feat': array([2048-d]), 'zone': None}}
        #         ...
        #           }
        # 'tid-j': {
        #     ...
        ...
        for tid in mot_list:
            ...
            if len(tracklet) <= 1: continue  # remove the object(TID) only appear once
            ...
            mean_feat = np.mean(all_feat, axis=0)  # 2048-d
